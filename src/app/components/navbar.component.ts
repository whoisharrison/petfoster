...
export class NavBarComponent implements AfterViewInit {
	...
	ngAfterViewInit(): void {
		$(".bd-example-modal-lg").on(
			"shown.bs.modal",
			() => this.renderTurnstile()
		);
	}

	private renderTurnstile(attempt: number = 0): void {
		...
		if(typeof turnstile === "undefined") {
			if(attempt < 40) {
				setTimeout(
					() => this.renderTurnstile(attempt + 1),
					250
				);
			}
	...
	createSignUp() : void {
		this.signUp.turnstileToken = null;

		if(
			this.turnstileWidgetId !== null &&
			typeof turnstile !== "undefined"
		) {
			this.signUp.turnstileToken =
