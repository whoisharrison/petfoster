<!DOCTYPE html>
<html lang="en">

	<head>
		<meta charset="utf-8">

		<title>Pet Foster ABQ - Persona-B</title>
		<style></style>
	</head>
	<body>
		<h1>Pet Foster ABQ</h1>
		<h2>Purpose, Audience, Goal</h2>

		<p></p><strong>Purpose:</strong> This project will be a professional website concentrating on adoption, foster placement, and the surrender of animals in order to better the lives of  animals as well as the owners.</p>
		<p><strong>Audience:</strong> This site will be designed and directed toward all animal lovers. We will focus on making sure that it appeals to a wide range of personalities including those with minimal technical skill.</p>
		<p><strong>Goal:</strong> The primary goal for this site is to garner recognition for animal welfare, provide placement for animals, and to allow owners to relinquish animals without scrutiny.</p>

		<h2>PERSONA</h2>
		<p><strong>Name:</strong> Chloe</p>
		<p><strong>Age:</strong> 10</p>
		<p><strong>Profession:</strong> Chloe is a sixth grade student.</p>

		<p><strong>Technology: </strong>Chloe has access to her families Apple Desk Top.</p>

		<p><strong>Attitudes and Behaviors: </strong></p>

		<p><strong>Frustrations and Needs:</strong> Last month Chloe found a small dog along the side of the road while walking home from school.  She brought the dog home and has been taking care of him.  Her father recently told her that she would have to find another home for the stray as they didn't have the time for life long placement. Chloe is frustrated as she hasn't been able to use the internet effectively to solve her issue.  She needs a simple site to place the stray on so that she can insure his safety.</p>

		<p><Strong>Goals:</Strong> <p></p>
	</body>


	Use Case: